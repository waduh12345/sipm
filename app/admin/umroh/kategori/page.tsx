...
} from "@/services/admin/umroh/kategori.service";
import type { Kategori } from "@/types/admin/umroh/kategori";
import FormUmrohKategori from "@/components/form-modal/admin/umroh/kategori-form";

export default function UmrohKategoriPage() {
  ...
  const { data, isLoading, refetch } = useGetUmrohKategoriListQuery({
    page: currentPage,
  ...
  const [createUmrohKategori, { isLoading: isCreating }] =
    useCreateUmrohKategoriMutation();
  const [updateUmrohKategori, { isLoading: isUpdating }] =
    useUpdateUmrohKategoriMutation();
  const [deleteUmrohKategori] = useDeleteUmrohKategoriMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await updateUmrohKategori({ id: editingId, payload }).unwrap();
        Swal.fire("Sukses", "Kategori Umroh diperbarui", "success");
      } else {
        await createUmrohKategori(payload).unwrap();
        Swal.fire("Sukses", "Kategori Umroh ditambahkan", "success");
      }
  ...
  const handleDelete = async (item: Kategori) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await deleteUmrohKategori(item.id).unwrap();
        await refetch();
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Kategori (Halaman Umroh)</h1>
