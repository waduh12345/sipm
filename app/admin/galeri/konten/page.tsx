...
} from "@/services/admin/galeri/konten.service";
import type { Konten } from "@/types/admin/galeri/konten";
import FormKontenGaleri from "@/components/form-modal/admin/galeri/konten-form";
import { getImageUrl } from "@/lib/image-url";

export default function GaleriKontenPage() {
  ...
  const { data, isLoading, refetch } = useGetGaleriKontenListQuery({
    page: currentPage,
  ...
  const [updateKonten, { isLoading: isUpdating }] =
    useUpdateGaleriKontenMutation();
  const [deleteKonten] = useDeleteGaleriKontenMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (form.tanggal) {
        const iso = /^\d{4}-\d{2}-\d{2}/.test(String(form.tanggal))
      ...
      if (form.image_4 instanceof File) fd.append("image_4", form.image_4);
      if (form.image_5 instanceof File) fd.append("image_5", form.image_5);
      if (form.image_6 instanceof File) fd.append("image_6", form.image_6);
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Konten galeri diperbarui", "success");
      } else {
        await createKonten(fd).unwrap();
        Swal.fire("Sukses", "Konten galeri ditambahkan", "success");
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Konten Galeri</h1>
