...
import { Kota } from "@/types/admin/master/kota";
import FormKota from "@/components/form-modal/admin/master/kota-form";
import { Input } from "@/components/ui/input";
import ActionsGroup from "@/components/admin-components/actions-group";
import { Plus } from "lucide-react";

export default function KotaPage() {
  const [form, setForm] = useState<Partial<Kota>>({
    id: "",
    province_id: "",
    name: ""
  });
  ...
  const { data, isLoading, refetch } = useGetKotaListQuery({
    page: currentPage,
  ...
  const [updateCategory, { isLoading: isUpdating }] =
    useUpdateKotaMutation();
  const [deleteCategory] = useDeleteKotaMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Kota diperbarui", "success");
      } else {
        await createCategory(payload).unwrap();
        Swal.fire("Sukses", "Kota ditambahkan", "success");
      }
  ...
  const handleDetail = (item: Kota) => {
    setForm(item);
    setReadonly(true);
    openModal();
  };

  const handleDelete = async (item: Kota) => {
    const confirm = await Swal.fire({
      title: "Yakin hapus Kota?",
      text: item.name,
      icon: "warning",
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        await refetch();
        Swal.fire("Berhasil", "Kota dihapus", "success");
      } catch (error) {
      ...
      <div className="rounded-md bg-white p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            ...
            {openModal && <Button onClick={openModal}><Plus /> Kota</Button>}
          </div>
        </div>
      </div>
