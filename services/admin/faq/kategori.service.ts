...
export const faqKategoriApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // 🔎 Get by ID
    getFaqKategoriById: builder.query<Kategori, number>({
      query: (id) => ({
    ...
    // ➕ Create (JSON)
    createFaqKategori: builder.mutation<Kategori, CreateKategoriPayload>({
      query: (payload) => ({
        url: `/website/faq/kategori`,
        method: "POST",
        body: payload,
    ...
    // ❌ Delete by ID
    deleteFaqKategori: builder.mutation<
