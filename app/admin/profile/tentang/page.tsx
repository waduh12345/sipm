...
} from "@/services/admin/profile/tentang.service";
import type { Tentang } from "@/types/admin/profile/tentang";
import FormTentang from "@/components/form-modal/admin/profile/tentang-form";
...
export default function TentangPage() {
  ...
  const { data, isLoading, refetch } = useGetProfileTentangListQuery({
    page: currentPage,
  ...
  const [createProfileTentang, { isLoading: isCreating }] =
    useCreateProfileTentangMutation();
  const [updateProfileTentang, { isLoading: isUpdating }] =
    useUpdateProfileTentangMutation();
  const [deleteProfileTentang] = useDeleteProfileTentangMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await updateProfileTentang({ id: editingId, payload: fd }).unwrap();
        Swal.fire("Sukses", "Data Tentang diperbarui", "success");
      } else {
        await createProfileTentang(fd).unwrap();
        Swal.fire("Sukses", "Data Tentang ditambahkan", "success");
      }
  ...
  const handleDelete = async (item: Tentang) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await deleteProfileTentang(item.id).unwrap();
        await refetch();
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Tentang (Profile)</h1>
