...
} from "@/services/admin/galeri/kategori.service";
import type { Kategori } from "@/types/admin/galeri/kategori";
import FormKategoriGaleri from "@/components/form-modal/admin/galeri/kategori-form";

export default function KategoriGaleriPage() {
  ...
  const { data, isLoading, refetch } = useGetGaleriKategoriListQuery({
    page: currentPage,
  ...
  const [updateKategori, { isLoading: isUpdating }] =
    useUpdateGaleriKategoriMutation();
  const [deleteKategori] = useDeleteGaleriKategoriMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Kategori Galeri diperbarui", "success");
      } else {
        await createKategori(payload).unwrap();
        Swal.fire("Sukses", "Kategori Galeri ditambahkan", "success");
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Kategori (Galeri)</h1>
