...
import { Level } from "@/types/admin/master/level";
import FormLevel from "@/components/form-modal/admin/master/level-form";
...
export default function LevelPage() {
  const [form, setForm] = useState<Partial<Level>>({
    code: "",
    name: "",
    order: "",
    description: "",
  ...
  const { data, isLoading, refetch } = useGetLevelListQuery({
    page: currentPage,
  ...
  const [updateCategory, { isLoading: isUpdating }] =
    useUpdateLevelMutation();
  const [deleteCategory] = useDeleteLevelMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Level diperbarui", "success");
      } else {
        await createCategory(payload).unwrap();
        Swal.fire("Sukses", "Level ditambahkan", "success");
      }
  ...
  const handleDelete = async (item: Level) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        Swal.fire("Gagal", "Gagal menghapus Level", "error");
        console.error(error);
      }
    }
  };
      ...
      <div className="rounded-md bg-white p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            ...
            {openModal && <Button onClick={openModal}><Plus /> Level</Button>}
          </div>
        </div>
      </div>
