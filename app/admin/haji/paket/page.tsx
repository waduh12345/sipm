...
} from "@/services/admin/haji/paket.service";
import type { Paket } from "@/types/admin/haji/paket";
import FormPaketHaji from "@/components/form-modal/admin/haji/paket-form";
import { getImageUrl } from "@/lib/image-url";

export default function HajiPaketPage() {
  ...
  const { data, isLoading, refetch } = useGetHajiPaketListQuery({
    page: currentPage,
  ...
  const [updatePaket, { isLoading: isUpdating }] =
    useUpdateHajiPaketMutation();
  const [deletePaket] = useDeleteHajiPaketMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Paket haji diperbarui", "success");
      } else {
        await createPaket(fd).unwrap();
        Swal.fire("Sukses", "Paket haji ditambahkan", "success");
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Paket Haji</h1>
